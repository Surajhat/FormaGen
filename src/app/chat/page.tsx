...
export default function ChatPage() {
  ...
  useEffect(() => {
    // Only redirect to home if there's no activity (no messages, not loading, no response, no query)
    if (messages.length === 0 && !isLoading && !currentResponse && !query.trim()) {
      router.push("/");
    }
  }, [messages, isLoading, currentResponse, query, router]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          {/* Section Header */}
          <div className={`px-3 py-3 border-b ${isDark ? "border-white/[0.04]" : "border-black/[0.04]"}`}>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3">
            <div className="space-y-3">
              {messages.length === 0 ? (
                <div className={`text-center py-12 px-4 ${isDark ? "text-white/30" : "text-black/30"}`}>
          ...
          {/* Input Area */}
          <div className={`flex-shrink-0 p-3 border-t ${isDark ? "border-white/[0.06]" : "border-black/[0.06]"}`}>
            <div
            ...
            >
              <textarea
                ...
                className={`w-full resize-none px-3 py-2.5 text-sm bg-transparent focus:outline-none leading-relaxed ${
        ...
        <main className={`flex-1 flex flex-col overflow-hidden ${isDark ? "bg-[#0d0d0d]" : "bg-[#f5f5f5]"}`}>
          {/* Prompt Header */}
          <div
            className={`flex-shrink-0 px-4 sm:px-6 lg:px-8 py-4 border-b ${
          ...
          <div className={`flex-1 overflow-y-auto ${isDark ? "bg-[#111111]" : "bg-white"}`}>
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold mb-3 ${isDark ? "text-white/60" : "text-black/60"}`}>
                    AI Response
                  </p>
                  <div
                  ...
                  >
                    {(selectedMessage?.response || currentResponse || isLoading) ? (
                      <ThemeProvider>
                        <C1Component
                          c1Response={
                            // When streaming, always show currentResponse (even if empty initially)
                            // When not streaming, show selected message response or currentResponse
                            isLoading ? (currentResponse || "") : (selectedMessage?.response || currentResponse || "")
                          }
                          isStreaming={isLoading}
                          updateMessage={(message) => {
                            // Only allow updates during streaming to prevent post-stream renders
                            if (isLoading) {
